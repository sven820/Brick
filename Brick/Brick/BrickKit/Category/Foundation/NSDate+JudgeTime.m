...
#import "NSDate+JudgeTime.h"
#import "NSString+Extension.h"

NSString *judgeWeek(NSInteger week){
    NSString *tempSting;
    switch (week) {
        case 1:
            tempSting=@"日";
...
@implementation NSDate (JudgeTime)
+(NSString *)judgeTimeWithTimeString:(NSString *)tempString
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat: @"yyyy-MM-dd"];
    dateFormatter.timeZone = [NSTimeZone localTimeZone];
    NSDate *tempDate= [dateFormatter dateFromString:tempString];
    
    NSCalendar *gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar];
    NSDateComponents *components = [gregorian components:(NSYearCalendarUnit | NSMonthCalendarUnit | NSDayCalendarUnit | NSWeekdayCalendarUnit) fromDate:tempDate];
...
//        if (day < 10)
//        {
//            timeString = [NSString stringWithFormat:@"0%ld-0%ld(周%@)",(long)month, (long)day, judgeWeek(week)];
//        }
//        else
//        {
//            timeString = [NSString stringWithFormat:@"0%ld-%ld(周%@)",(long)month, (long)day, judgeWeek(week)];
//        }
//    }
//    else
//    {
//        if (day < 10)
//        {
//            timeString = [NSString stringWithFormat:@"%ld-0%ld(周%@)",(long)month, (long)day, judgeWeek(week)];
//        }
//        else
//        {
//            timeString = [NSString stringWithFormat:@"%ld-%ld(周%@)",(long)month, (long)day, judgeWeek(week)];
//        }
//    }
    
    tempString = [NSString stringWithFormat:@"%@(周%@)",tempString, judgeWeek(week)];
...
+ (NSDate *)dateFromDefaultFormatedString:(NSString *)string
{
    if (![NSString checkIsEmptyOrNull:string])
    {
        NSDate *date = [NSDate dateFromString:string withFormat:@"yyyy-MM-dd HH:mm:ss"];

        if (date == nil)
        {
            date = [NSDate dateFromString:string withFormat:@"yyyy-MM-dd HH:mm"];
            ;
        }
        if (date == nil)
        {
            date = [NSDate dateFromString:string withFormat:@"yyyy-MM-dd"];
            ;
        }
        return date;
    }
